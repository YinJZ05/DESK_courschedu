...
支持待办事项管理，可在设置中开关待办模块，并在课表与待办之间连续拖动分隔线调整显示高度。
...
- 勾选复选框标记完成/未完成。
- 右键待办可 编辑 或 删除。
3. 在课表与待办之间拖动分隔线（中间的 ▲▼ 提示条）可线性连续调整两块区域高度。
4. 在设置中可调整：
- 开机自启动
- 启用课表功能
- 启用待办功能
...
- data/todo_items.json

说明：
- settings.json 保存窗口与功能设置（包含课表/待办开关、待办面板高度等）。
- learned_progress.json 保存你手动维护的已上课数。
- todo_items.json 保存待办内容与完成状态。
...
持久化文件：
1. data/settings.json：窗口尺寸、课程显隐、课表/待办开关、待办面板高度、启动偏好、最近刷新日期。
2. data/learned_progress.json：用户手动维护的已上课数。
3. data/todo_items.json：待办内容及完成状态。
